...
$$ MAX \ \sum \sum V_{i}X_{i}$$ \### Restrições

$$  \sum P_{i}X_{i} \leq  \ W_{j}$$ \### Domínio das Variáveis
...
Suponha que tenha uma mochila com capacidade total de $W$ e $n$ itens distintos. Seja $x_{i},...,x_{n}$ a quantidade de itens que está sendo carregado, cada um com um respectivo peso $w_{i},...,w_{n}$ e valor $v_{1},...,v_{n}$. Para Maximizar o valor da mochila é usando a seguinte equação:

$$ \sum_{i=1}^{n} v_{i}x_{i}, \ \ \ sujeito \ a \ \ \sum_{i=1}^{n} w_{i}x_{i} \ \ \leq W$$
